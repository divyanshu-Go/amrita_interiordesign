// lib/fetchers/newCategoryFetcher.js
//
// ── PURPOSE ───────────────────────────────────────────────────────────────
...
async function fetchFilterOptions(categoryId) {
  ...
  const [agg] = await Product.aggregate([
    { $match: matchStage },
    // Flatten array fields (material/pattern/finish) to individual values
    // BEFORE grouping, so $addToSet dedupes everything the same way —
    // no separate JS flatten/dedupe pass needed afterward.
...
// ── Lightweight category fetch for generateMetadata ────────────────────────
// Metadata only needs name/description/seoIntro/image — not the full
// filter-options aggregation or product query. Keeping this separate
// avoids doing unnecessary work on every metadata generation.
