...
// Fetches the first `limit` products for a category to render server-side.
// Purpose: give Googlebot real product names + prices in the initial HTML.
//
// Called from the category page SERVER component only.
// The client component (CategoryPageClient) still handles all interactive
// fetching — this is purely for the SSR snapshot Google sees.
// ──────────────────────────────────────────────────────────────────────────
export async function getSSRCategoryProducts(categoryId, limit = 8) {
  await DbConnect();
  try {
    const products = await Product.find({ category: categoryId })
      .select(
        "name slug images retailPrice retailDiscountPrice " +
        "enterprisePrice enterpriseDiscountPrice " +
        "sellBy showPerSqFtPrice perSqFtPriceRetail color brand"
      )
    ...
    console.error("[getSSRCategoryProducts] error:", err);
    return [];
  }
}